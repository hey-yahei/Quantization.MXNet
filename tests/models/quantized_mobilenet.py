...
import sys
sys.path.append("../..")
from nn.quantized_conv import Conv2D as QConv2D
...
def _add_conv(out, channels=1, kernel=1, stride=1, pad=0, in_channels=3,
              num_group=1, active=True, relu6=False, norm_layer=BatchNorm, norm_kwargs=None, quantized=True):
    if quantized:
        out.add(QConv2D(channels, kernel, stride, pad, in_channels=in_channels, groups=num_group, use_bias=False,
                        quantized=quantized, input_dtype="uint8", weight_dtype="int8"))
    else:
        out.add(QConv2D(channels, kernel, stride, pad, in_channels=in_channels,groups=num_group, use_bias=False))
...
def _add_conv_dw(out, dw_channels, channels, stride, relu6=False,
    ...
    _add_conv(out, channels=dw_channels, kernel=3, stride=stride, in_channels=dw_channels,
              pad=1, num_group=dw_channels, relu6=relu6,
              norm_layer=BatchNorm, norm_kwargs=None)
    _add_conv(out, channels=channels, relu6=relu6, in_channels=dw_channels,
              norm_layer=BatchNorm, norm_kwargs=None)
    ...
    def __init__(self, multiplier=1.0, classes=1000,
        ...
        with self.name_scope():
            ...
            with self.output.name_scope():
                self.output.add(
                    QConv2D(classes, 1, use_bias=False, prefix='pred_'),
                    nn.Flatten()
                )
